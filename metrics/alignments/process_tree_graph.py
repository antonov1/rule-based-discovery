...
class ProcessTreeGraph(nx.MultiDiGraph):
    ...
    def _build_process_tree_subgraph(
        self, tree: ProcessTree, start_node: Hashable, end_node: Hashable, iac: int = 1
    ) -> None:
        match tree.operator:
            case None:
                self._build_process_tree_subgraph_leaf(tree, start_node, end_node, iac)
            case Operator.SEQUENCE:
                self._build_process_tree_subgraph_sequence(
                    tree, start_node, end_node, iac
                )
            case Operator.XOR:
                self._build_process_tree_subgraph_xor(tree, start_node, end_node, iac)
            case Operator.PARALLEL:
                self._build_process_tree_subgraph_parallel(
                    tree, start_node, end_node, iac
                )
            case Operator.LOOP:
    ...
    def _build_process_tree_subgraph_leaf(
        self, tree: ProcessTree, start_node: Hashable, end_node: Hashable, iac: int = 1
    ) -> None:
        if not is_leaf(tree):
    ...
    def _build_process_tree_subgraph_sequence(
        self, tree: ProcessTree, start_node: Hashable, end_node: Hashable, iac: int = 1
    ) -> None:
        ...
        if len(tree.children) == 1:
            self._build_process_tree_subgraph(
                tree.children[0], start_node, end_node, iac
            )
            return
        ...
        self._build_process_tree_subgraph(
            tree.children[0], start_node, right_node, iac=iac
        )
        for i in range(1, len(tree.children) - 1):
            ...
            self.add_node(right_node)
            self._build_process_tree_subgraph(
                tree.children[i], left_node, right_node, iac=iac
            )
        self._build_process_tree_subgraph(
            tree.children[-1], right_node, end_node, iac=iac
        )

    def _build_process_tree_subgraph_xor(
        self, tree: ProcessTree, start_node: Hashable, end_node: Hashable, iac: int = 1
    ) -> None:
    ...
    def _build_process_tree_subgraph_parallel(
        self, tree: ProcessTree, start_node: Hashable, end_node: Hashable, iac: int = 1
    ) -> None:
    ...
    def _build_process_tree_subgraph_loop(
        ...
        self.add_edge(
            old_start_node, start_node, label=None, capacity=1.0 / iac, cost=0
        )
        old_end_node = end_node
        end_node = self.number_of_nodes()
        self.add_edge(end_node, old_end_node, label=None, capacity=1.0 / iac, cost=0)
    ...
    def view(
        self, rotate: bool = True, rad: float = 0.0, rad_offset: float = 0.0
    ) -> None:
        ...
        nx.draw_networkx_nodes(
            ...
            node_color=[
                (
                    "g"
                    if "source" in (k := self.nodes.get(n).keys())
        ...
        nx.draw_networkx_labels(
            self, pos, font_size=10, font_color="w", font_family="sans-serif"
        )
        ...
        for e in self.edges:
            if e[0] == e[1]:
                ...
                conn = ConnectionPatch(
                    ...
                    patchB=None,
                    connectionstyle=(
                        lambda posA, posB, *args, **kwargs: Path(
                ...
                ctrl = (
                    0.5 * pos_0
                    + 0.5 * pos_1
                    + (rad + rad_offset * e[2]) * np.array([d_pos[1], -d_pos[0]])
                )  # np.array([(0, 1), (-1, 0)]) @ d_pos
                ...
                def get_bezier_point_and_angle(t: float) -> tuple[np.ndarray, float]:
                    ...
                    angle = np.rad2deg(
                        np.arctan((ctrl_1[1] - ctrl_0[1]) / (ctrl_1[0] - ctrl_0[0]))
                    )
                    return bezier, angle
            ...
            ax.text(
                ...
                ha="center",
                va="center",
                rotation=angle1 if rotate else 0.0,
            ...
            ax.text(
                ...
                ha="center",
                va="center",
                rotation=angle2 if rotate else 0.0,
                transform=ax.transData,
                bbox=bbox,
                zorder=1,
            )
            ax.text(
                ...
                ha="center",
                va="center",
                rotation=angle3 if rotate else 0.0,
                transform=ax.transData,
                bbox=bbox,
                zorder=1,
            )
    ...
    def view_pyvis(self) -> None:
        ...
        for node in self.nodes:
            ...
            net.add_node(
                ...
                color=(
                    "green"
                    if "source" in self.nodes.get(node).keys()
                    else (
                        "red"
                        if "sink" in self.nodes.get(node).keys()
                        else (
                            "blue"
                            if "shuffle" in self.nodes.get(node).keys()
                            else "gray"
                        )
                    )
                ),
        ...
        for edge in self.edges:
            net.add_edge(
                edge[0],
                edge[1],
                label=self.edges.get(edge)["label"],
