...
def align_multithreaded(
    trace: Trace, process_tree_graph: ProcessTreeGraph, env: gp.Env
) -> float:
    with gp.Model("process_tree_alignment", env=env) as m:
        ...
        z = m.addVars(
            [(i, *e) for i in sync_edges for e in sync_edges[i]],
            lb=0,
            ub={
                (i, *e): process_tree_graph.edges.get(e).get("capacity")
                for i in sync_edges
            ...
            obj={
                (i, *e): (
                    1 - cost
                    if (cost := process_tree_graph.edges.get(e).get("cost")) > 1
                    else 0
                )
                for i in sync_edges
                for e in sync_edges[i]
        ...
        if len(trace) == 0:
            m.addConstrs(
                ...
                == (
                    0
                    if process_tree_graph.nodes.get(v).get("source")
            ...
            m.addConstrs(
                ...
                == (
                    -1
                    if i == 0 and process_tree_graph.nodes.get(v).get("source")
                    else (
                        1
                        if i == len(trace)
                        and process_tree_graph.nodes.get(v).get("sink")
                        else 0
                    )
                )
                for i in range(len(trace) + 1)
        ...
        shuffles = {
            (k, i): w
            for k, v in process_tree_graph.nodes._nodes.items()
            if v.get("shuffle")
            for i, w in enumerate(v["shuffle"])
        }
        s = m.addVars(len(trace) + 1, shuffles.keys(), vtype=GRB.BINARY, name="s")
        m.addConstrs(
            gp.quicksum(x[(i, *e)] for e in shuffles[(v, j)])
        ...
        m.addConstrs(
            gp.quicksum(
                z[(i, *e)] * process_tree_graph.edges.get(e).get("cost")
                for e in sync_edges[i]
            )
            <= 1
            for i in sync_edges
