...
class CoExistenceRule(AbstractRule):
    def __init__(self, name: String, activities: List[String]) -> None:
        if len(activities) != 2:
            raise ValueError("CoExistenceRule must have exactly two activities.")
    ...
    def get_confidence(self, data) -> float:
        ...
        count_ab = sum(1 for trace in self.valid_traces if self.activity_a in trace and self.activity_b in trace)
        ...
        return max(count_ab / count_a, count_ab / count_b)